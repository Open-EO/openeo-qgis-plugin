        ...
        for elem in caps.findall(_TILE_MATRIX_SET_TAG):
            ...
            if tms.identifier:
                if tms.identifier in self.tilematrixsets:
                    msg = (
                        'TileMatrixSet with identifier "%s" '
        ...
        for elem in self._capabilities.findall(_THEMES_TAG + "/" + _THEME_TAG):
            ...
            if theme.identifier:
                if theme.identifier in self.themes:
                    msg = (
                        'Theme with identifier "%s" already exists'
                        % theme.identifier
                    )
                    warnings.warn(msg, RuntimeWarning)
                        ...
                        for x in filter(
                            ...
                            (
                                [
                                    ...
                                    if "kvp"
                                    in [x.lower() for x in const.values]
                                ]
                                for pv in get_verbs
                                if pv.get("constraints")
                            ),
                        )
                    )[0]
...
class TileMatrixLimits(object):
    ...
    def __repr__(self):
        fmt = (
            "<TileMatrixLimits: {self.tilematrix}"
...
class TileMatrixSetLink(object):
        ...
        for link_element in link_elements:
            ...
            if len(matrix_set_elements) == 0:
                ...
                if set_limits_elements:
                    raise ValueError(
                        "Multiple instances of TileMatrixSet"
                        " plus TileMatrixSetLimits in %s" % link_element
                    )
                ...
                for limits_element in link_element.findall(path):
                    ...
                    if tml.tilematrix:
                        if tml.tilematrix in tilematrixlimits:
                            msg = (
                                'TileMatrixLimits with tileMatrix "%s" '
    ...
    def __repr__(self):
        fmt = (
            "<TileMatrixSetLink: {self.tilematrixset}"
            ", tilematrixlimits={{...}}>"
        )
        return fmt.format(self=self)
...
class BoundingBox(object):
    ...
    def __repr__(self):
        fmt = "<BoundingBox, crs={self.crs}, extent={self.extent}>"
        return fmt.format(self=self)
...
class ContentMetadata:
    ...
    def __init__(
        self, elem, parent=None, index=0, parse_remote_metadata=False
    ):
        ...
        self._tilematrixsets = [
            ...
            for f in elem.findall(
                _TILE_MATRIX_SET_LINK_TAG + "/" + _TILE_MATRIX_SET_TAG
            )
        ]
        ...
        for tmsl in tile_matrix_set_links:
            if tmsl.tilematrixset:
                if tmsl.tilematrixset in self.tilematrixsetlinks:
                    msg = (
                        'TileMatrixSetLink with tilematrixset "%s"'
        ...
        for s in elem.findall(_STYLE_TAG):
            ...
            keywords = [
                f.text for f in s.findall(_KEYWORDS_TAG + "/" + _KEYWORD_TAG)
            ]
        ...
        self.keywords = [
            f.text for f in elem.findall(_KEYWORDS_TAG + "/" + _KEYWORD_TAG)
        ]
        ...
        for dim in elem.findall(_DIMENSION_TAG):
            ...
            if identifier.text in self.dimensions:
                ...
                warnings.warn(
                    "%s identifier duplicated, taking first occurence" % (dim,)
                )
                continue

            values = [f.text for f in dim.findall(_DIMENSION_VALUE_TAG)]
            if len(values) == 0:
                # ADD: Consider the default value if no value is set
                # This is not WMTS compliant, but helps with reading some WMTS that are not compliant
                default_ = dim.find(_DIMENSION_DEFAULT_TAG)
                if default_ is not None:
                    values = [default_.text]
                else:
                    raise ValueError(
    ...
    def tilematrixsets(self):
        ...
        warnings.warn(
            "The 'tilematrixsets' attribute has been deprecated"
